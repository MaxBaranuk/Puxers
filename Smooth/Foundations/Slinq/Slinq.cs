...
using Smooth.Foundations.Slinq.Context.ChainedOrPooled;
using Smooth.Pools;
using Smooth.Slinq.Collections;
using Smooth.Slinq.Context;

namespace Smooth.Foundations.Slinq {
	...
	public struct Slinq<T, C> {
		...
		/// This operation will consume and dispose the Slinq.
		/// </summary>
		public Algebraics.Tuple<LinkedHeadTail<T>, LinkedHeadTail<T>> SplitRight(int count) {
			if (current.isSome) {
				if (count > 0) {
					...
					return new Algebraics.Tuple<LinkedHeadTail<T>, LinkedHeadTail<T>>(left, right);
				} else {
					return new Algebraics.Tuple<LinkedHeadTail<T>, LinkedHeadTail<T>>(ToLinked(), new LinkedHeadTail<T>());
				}
			} else {
				return new Algebraics.Tuple<LinkedHeadTail<T>, LinkedHeadTail<T>>();
			}
		}
	...
	public static class Slinq {
		...
		/// </summary>
		public static Slinq<Algebraics.Tuple<Option<T>, Option<T2>>, ZipAllContext<T2, C2, T, C>> ZipAll<T2, C2, T, C>(this Slinq<T, C> slinq, Slinq<T2, C2> with) {
		...
		/// Removal operations will not be chained to an empty Slinq.
		/// </summary>
		public static Slinq<Algebraics.Tuple<Option<T>, Option<T2>>, ZipAllContext<T2, C2, T, C>> ZipAll<T2, C2, T, C>(this Slinq<T, C> slinq, Slinq<T2, C2> with, ZipRemoveFlags removeFlags) {
